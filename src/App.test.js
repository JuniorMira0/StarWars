...
describe('7 - Ordene as colunas de forma ascendente ou descendente', () => {
  ...
  it('Ordene os planetas do mais populoso para o menos populoso', async () => {
    ...
    const expectedPlanetsWithNumericValues = [
      'Coruscant',
      'Naboo',
      'Alderaan',
      'Kamino',
    ...
    const actualPlanetsWithNumericValues = planets
      .map((planet) => planet.innerHTML)
      .slice(0, 8);
    expect(actualPlanetsWithNumericValues).toEqual(
      expectedPlanetsWithNumericValues
    );

    const expectedPlanetsWithUnknownValues = ['Dagobah', 'Hoth'];
    const actualPlanetsWithUnknownValues = planets
      .map((planet) => planet.innerHTML)
      .slice(8, 10);
    expect(actualPlanetsWithUnknownValues).toEqual(
      expect.arrayContaining(expectedPlanetsWithUnknownValues)
    );
  });
});
